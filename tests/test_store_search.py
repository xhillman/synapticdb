...
def remember(store: Store, content: str, embedding: tuple[float, ...]) -> UUID:
    return store.insert_memory(content, {}, embedding).id


def test_keyword_search_uses_bm25_ordering(store: Store) -> None:
    frequent_id = remember(store, "alpha alpha alpha alpha", (1.0, 0.0))
    sparse_id = remember(store, "alpha beta gamma delta epsilon zeta eta theta", (0.8, 0.2))
    remember(store, "beta only", (0.0, 1.0))
...
def test_keyword_search_handles_plain_punctuation(store: Store) -> None:
    alpha_id = remember(store, "alpha record", (1.0, 0.0))
    beta_id = remember(store, "beta record", (0.0, 1.0))
...
def test_fts_stays_synced_after_insert_update_and_delete(store: Store, store_path: Path) -> None:
    memory_id = remember(store, "original token", (1.0, 0.0))
...
def test_semantic_search_orders_by_cosine_similarity(store: Store) -> None:
    exact_id = remember(store, "exact", (1.0, 0.0))
    near_id = remember(store, "near", (0.8, 0.2))
    orthogonal_id = remember(store, "orthogonal", (0.0, 1.0))
...
def test_vector_cache_updates_after_insert_and_forget(store: Store) -> None:
    positive_id = remember(store, "positive", (1.0, 0.0))
    store.semantic_search((1.0, 0.0))
    negative_id = remember(store, "negative", (-1.0, 0.0))
